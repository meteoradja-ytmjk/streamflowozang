...
async function buildFFmpegArgsWithAudio(stream, videoPath, audioPath, rtmpUrl) {
  ...
  console.log(`[StreamingService] Merging video+audio: Audio=${audioDuration}s, Video=${videoDuration}s, Loops=${loopCount}`);
  addStreamLog(stream.id, `Merging video (${videoDuration}s) with audio (${audioDuration}s), video will loop ${loopCount} times`);
...
async function buildFFmpegArgs(stream) {
  ...
  // Check if audio is merged with video
  let audioPath = null;
  if (stream.audio_id) {
    ...
    if (audio) {
      ...
      if (!fs.existsSync(audioPath)) {
        console.error(`[StreamingService] Audio file not found: ${audioPath}`);
        audioPath = null;
      } else {
        console.log(`[StreamingService] Using merged audio: ${audioPath}`);
      }
    }
  }
  ...
  // If audio is merged, use different FFmpeg args
  if (audioPath) {
    return await buildFFmpegArgsWithAudio(stream, videoPath, audioPath, rtmpUrl);
  }
...
async function stopStream(streamId) {
  try {
    ...
    // Force kill after 2 seconds if process still exists
    setTimeout(() => {
      if (activeStreams.has(streamId)) {
        try {
          ...
          console.error(`[StreamingService] Error sending SIGKILL: ${forceKillError.message}`);
        }
      }
    }, 2000);
    ...
    // Update stream status to offline immediately
    if (stream) {
      await Stream.updateStatus(streamId, 'offline', stream.user_id);
    ...
    // Cancel any scheduled terminations
    if (typeof schedulerService !== 'undefined' && schedulerService.handleStreamStopped) {
      schedulerService.handleStreamStopped(streamId);
    }
